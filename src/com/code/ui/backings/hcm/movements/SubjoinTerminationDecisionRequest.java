 ...
 * 
 */
@ManagedBean(name = "subjoinTerminationDecisionRequest")
@ViewScoped
public class SubjoinTerminationDecisionRequest extends MovementsBase implements Serializable {
    /**
     * 1 for Officers, 2 for Soldiers and 3 for Persons
     **/

    public SubjoinTerminationDecisionRequest() {
	super.init();
	if (getRequest().getParameter("mode") != null) {
	    ...
	    switch (mode) {
	    case 1:
		setScreenTitle(getMessage("title_officersSubjoinTerminationDecisionRequest"));
		this.processId = WFProcessesEnum.OFFICERS_SUBJOIN_TERMINATION.getCode();
		break;
	    case 2:
		setScreenTitle(getMessage("title_soldiersSubjoinTerminationDecisionRequest"));
		this.processId = WFProcessesEnum.SOLDIERS_SUBJOIN_TERMINATION.getCode();
		break;
	    case 3:
		setScreenTitle(getMessage("title_personsInternalAssignmentTerminationDecisionRequest"));
	...
	try {
		...
		for (WFMovementData wFMovementdata : wfMovementsList)
		    wFMovementdata.setEndDate(HijriDateService.getHijriSysDate());
	    }
	} catch (BusinessException e) {
	    setServerSideErrorMessages(getMessage(e.getMessage()));
	}
