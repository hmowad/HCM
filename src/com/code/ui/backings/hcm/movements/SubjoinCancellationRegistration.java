...
@ManagedBean(name = "subjoinCancellationRegistration")
@ViewScoped
public class SubjoinCancellationRegistration extends BaseBacking implements Serializable {
    /**
    ...
    public SubjoinCancellationRegistration() {
	if (getRequest().getParameter("mode") != null) {
	    mode = Integer.parseInt(getRequest().getParameter("mode"));
	    this.init();
	    switch (mode) {
	    case 1:
		setScreenTitle(getMessage("title_officersExternalSubjoinCancellationRegistration"));
		break;
	    case 3:
	...
	try {
	    commonSubjoinData = new MovementTransactionData();
	    commonSubjoinData.setDecisionDate(HijriDateService.getHijriSysDate());
	    subjoinList = MovementsService.constructMovementTransactionsByDecisionInfo(basedOnDecisionNumber, basedOnDecisionDate, getCategoriesIdsArrayByMode(mode), MovementTypesEnum.SUBJOIN.getCode(), TransactionTypesEnum.MVT_CANCEL_DECISION.getCode(), FlagsEnum.OFF.getCode());
	} catch (BusinessException e) {
