...
@ManagedBean(name = "scholarshipClaimRequest")
@ViewScoped
public class ScholarshipClaimRequest extends TrainingAndScholarshipBase {
    private List<QualificationLevel> qualificationLevels;

    public ScholarshipClaimRequest() {
	try {
	    super.init();
	    setScreenTitle(getMessage("title_scholarshipClaimRequest"));
		...
		if (SecurityService.isEmployeeMenuActionGranted(this.loginEmpData.getEmpId(), MenuCodesEnum.TRN_SCHOLARSHIP_CLAIM_REQUEST.getCode(), MenuActionsEnum.TRN_CLAIMS_REQUEST.getCode()))
		    this.processId = WFProcessesEnum.SCHOLARSHIP_CLAIM.getCode();
		else
		    this.processId = WFProcessesEnum.SCHOLARSHIP_CLAIM_REQUEST.getCode();
	    ...
	    if (this.processId == WFProcessesEnum.SCHOLARSHIP_CLAIM.getCode())
