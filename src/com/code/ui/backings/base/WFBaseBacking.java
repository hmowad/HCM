...
public abstract class WFBaseBacking extends BaseBacking implements Serializable {
	...
	public String approveEserviceTransaction(Long id) {
		try {
			EServicesBaseWorkFlowService.doApproval(requester.getEmpId(), currentTask, requester.getEmpId(), null, WFTaskActionsEnum.SUPER_SIGN, id);
	...
	public String rejectEserviceTransaction(Long id) {
		try {
			EServicesBaseWorkFlowService.doApproval(requester.getEmpId(), currentTask, requester.getEmpId(), null, WFTaskActionsEnum.REJECT, id);
			this.setServerSideSuccessMessages(getParameterizedMessage("notify_successOperation"));
			return NavigationEnum.OUTBOX.toString();
		} catch (BusinessException e) {
			setServerSideErrorMessages(getMessage(e.getMessage()));
