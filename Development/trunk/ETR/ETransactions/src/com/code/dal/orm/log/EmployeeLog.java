...
@NamedQueries({
	@NamedQuery(name = "hcm_employeeLog_getLastEmployeeLog",
		query = "select e from EmployeeLog e " +
			" where e.effectiveHijriDate = ( select max(e1.effectiveHijriDate) from EmployeeLog e1 where e1.empId = :P_EMP_ID and e1.effectiveHijriDate < (TO_DATE(:P_EFFECTIVE_HIJRI_DATE, 'MI/MM/YYYY')) ) " +
			" and e.empId = :P_EMP_ID " +
			" and e.insertionTime = (select max(e2.insertionTime) from EmployeeLog e2 where e2.empId = :P_EMP_ID and e2.effectiveHijriDate < (TO_DATE(:P_EFFECTIVE_HIJRI_DATE, 'MI/MM/YYYY'))) ")
})
