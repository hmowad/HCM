...
@ManagedBean(name = "subjoinTerminationRegistration")
@ViewScoped
public class SubjoinTerminationRegistration extends BaseBacking implements Serializable {
    /**
    ...
    public SubjoinTerminationRegistration() {
	if (getRequest().getParameter("mode") != null) {
	    mode = Integer.parseInt(getRequest().getParameter("mode"));
	    this.init();
	    switch (mode) {
	    case 1:
		setScreenTitle(getMessage("title_officersExternalSubjoinTerminationRegistration"));
		break;
	    case 3:
	...
	try {
	    commonSubjoinData = new MovementTransactionData();
	    commonSubjoinData.setDecisionDate(HijriDateService.getHijriSysDate());
	    subjoinList = MovementsService.constructMovementTransactionsByDecisionInfo(basedOnDecisionNumber, basedOnDecisionDate, getCategoriesIdsArrayByMode(mode), MovementTypesEnum.SUBJOIN.getCode(), TransactionTypesEnum.MVT_TERMINATION_DECISION.getCode(), FlagsEnum.OFF.getCode());
	    for (MovementTransactionData movementTransactionData : subjoinList)
		movementTransactionData.setEndDate(HijriDateService.getHijriSysDate());
	} catch (BusinessException e) {
